...
| Fractional  | [Fractional — Missing ERC2981 Interface Declaration Breaks Royalty Detection](/bugs/business-logic-flaw/fractional-missing-erc2981-interface-royalty-break.md)  | Medium  | Code4rena |
...
| Monetrix  | [Phantom Surplus Settlement via Omitted Portfolio Margin Borrow Liabilities](/bugs/business-logic-flaw/monetrix-pm-borrow-liabilities-omitted-from-backing-phantom-surplus-settlement.md) | Medium  | Code4rena |
...
| Infinity  | [Arbitrary WETH Drain via Attacker-Controlled Gas Consumption in NFT Transfers](/bugs/external-call/infinityexchange-arbitrary-gas-refund-drain-via-malicious-nft.md) | Medium  | Code4rena |
...
| Frankencoin | [Challenge Sandwich Attack via Frontrunnable Price Manipulation](/bugs/frontrunning-and-mev/frankencoin-challenge-sandwich-attack-via-price-manipulation.md)  | High  | Code4rena |
...
| Chainlink Payment Abstraction V2  | [ERC-1271 Signer Does Not Pin `appData`, Allowing Solver-Controlled CoW Surplus Siphoning](/bugs/invalid-validation/chainlink-payment-abstraction-v2-unpinned-appdata-allows-cow-surplus-siphoning.md)  | Medium  | Code4rena |
...
| Init Capital  | [Arbitrary `tokenOut` Manipulation via Missing Validation in `updateOrder`](/bugs/invalid-validation/init-capital-missing-tokenout-validation-on-order-update.md) | High  | Code4rena |
| Intuition | [Unsigned Validity Window Metadata in AtomWallet Signature Validation](/bugs/invalid-validation/intuition-unsigned-validity-window-metadata-in-atomwallet-signature-validation.md)  | Medium  | Code4rena |
...
| Phi | [Unit Mismatch in Fee Validation Prevents Protocol Fee Updates](/bugs/invalid-validation/phi-unit-mismatch-fee-validation-locks-protocol-fee-configuration.md)  | Medium  | Code4rena |
...
| Battlechain Confidence Pools |  [Moderator Grace Can Expire Before `CORRUPTED` Exists](/bugs/timing/battlechain-confidence-pools-moderator-grace-expires-before-corrupted-exists.md)  | Medium  | Codehawks |
...
| Notional  | [Emergency Settlement Bypass via Inflated BPT Supply in Balancer Phantom Pool](/bugs/other/notional-inflated-bpt-supply-prevents-emergency-settlement.md) | Logic Error / Incorrect Accounting / Risk-Control Bypass  | High  | Sherlock  |
| OpenDollar  | [Missing Token Existence Check Causes ERC-721 Metadata Specification Violation](/bugs/other/opendollar-missing-token-existence-check-erc721-specification-violation.md) | Standards Compliance / Missing Validation / ERC-721 Metadata  | Medium  | Code4rena |
